...
condition is metastable closure. STAK-PSAL detects it via $\mathcal{F}_n$,
a cumulative fatigue metric invisible to instantaneous monitors. A formal
layer (machine-checked Coq proofs of admissibility invariants, the
contraction bridge, and the structural separation between operational
closure and metastable closure) grounds an operational
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{fig_blindspot.pdf}
...
closed while carrying a bounded rupture horizon. \emph{First}, structural
divergence under operational equivalence is machine-checked in the
...
(Section~\ref{sec:opapprox}). \emph{Third}, $\mathcal{F}_n$ is
...
The monitor is scoped to continuous degradation; applicability beyond
this regime is an open boundary (Section~\ref{sec:limits}).
Table~\ref{tab:comparison-design} situates the contribution.

\begin{table}[H]
\centering
\caption{Existing monitors and their blind spot for distinguishability collapse}
\label{tab:comparison-design}
\small
\begin{tabular}{ll>{\raggedright\arraybackslash}p{.32\linewidth}}
\toprule
\textbf{Approach} & \textbf{What it detects} & \textbf{What it misses} \\
\midrule
...
\subsection{The Enrichment Response as Observational Resolution Enhancement}
...
The weight-boost is one instance of a broader family of observational
resolution enhancements; Table~\ref{tab:enrichment} lists mechanisms by
deployment class. The theoretical content is independent of the
particular mechanism.

\begin{table}[H]
\centering
\caption{Enrichment mechanisms by deployment class}
\label{tab:enrichment}
\small
\begin{tabular}{ll>{\raggedright\arraybackslash}p{.34\linewidth}}
\toprule
\textbf{Deployment class} & \textbf{Mechanism} & \textbf{Realisation} \\
\midrule
...
For adversarial or certified deployments, the immutable variants in
Table~\ref{tab:enrichment} are not optional engineering alternatives but
the required deployment models: mutable matrix modification is appropriate
only in trusted demonstrators or authenticated upgradeable environments.
The formal admissibility obligations are unchanged under these
substitutions, although each deployment mechanism requires its own
implementation proof.

Enrichment restores $\varepsilon$-admissibility if $d_t$ rises above
$\emeta$; sufficient boost ($b \geq b_{\mathrm{crit}}(\delta)$) sustains
it indefinitely, insufficient boost delays but does not prevent rupture.
...
Table~\ref{tab:vocab} gives the RV reading of each term.

\begin{table}[H]
\centering
\caption{STAK-PSAL terminology in standard RV reading}
\label{tab:vocab}
\small
\begin{tabular}{ll}
\toprule
\textbf{STAK-PSAL term} & \textbf{RV reading} \\
\midrule
$\varepsilon$-admissibility  & runtime-checkable separation invariant \\
Rupture                      & monitor indistinguishability failure \\
Metastable closure           & confirmed recovery with accumulated admissibility deficit \\
\st{critical} forecast       & anticipatory violation signal; leading indicator \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{proof}[Sketch]
Multiplicative decay of $W_t$ gives $|W_t(x - y)| \propto (1-\delta)^{t'
- t}$. Lipschitz $\sigma$ yields $d_{t'} \leq (1-\delta)^{t' - t} d_t /
c$ for $c$ absorbing the Lipschitz factor and projection geometry;
solving $(1-\delta)^T \varepsilon \leq \varepsilon'$ gives the bound.
\end{proof}
...
refinement isolating the precise horizon.
...
The monitor is implemented in OCaml~4.14; formal invariants are
machine-checked in Coq~8.18. Table~\ref{tab:modules} lists module
responsibilities.
...
\label{tab:modules}
\small
\begin{tabular}{l>{\raggedright\arraybackslash}p{.58\linewidth}}
\toprule
\textbf{Module} & \textbf{Responsibility} \\
\midrule
\texttt{types.ml}
  & Core types and status variants \\
\texttt{lnp.ml}
  & Reduction $M_t = \sigma(W_t \cdot z)$; degradation; enrichment \\
\texttt{admissibility.ml}
  & $\Phi$; three-way check (\texttt{Admissible} / \texttt{Drifting} / \texttt{HardRupture}) \\
\texttt{tension.ml}
  & Estimators: $\bar{d}_t$, $R_t$, $\dot{T}_t$ \\
\texttt{machine.ml}
  & State transitions; $E_r^{(t)}$; $\mathcal{F}_n$ \\
\texttt{instability.ml}
  & \st{stable}/\st{strained}/\st{critical} classification \\
\texttt{certificate.ml}
  & Rupture certificate builder and JSON serialiser \\
\texttt{scenario.ml}
  & Four-modality industrial benchmark scenario \\
\texttt{formal/stak\_psal.v}
  & Coq proofs of invariants (T1)--(T5) of Theorem~\ref{thm:coq} \\
\texttt{formal/contraction\_core.v}
  & Coq proof of geometric multi-step contraction lemma \\
\texttt{formal/threshold\_bridge.v}
  & Coq proof of finite $\varepsilon$-to-$\varepsilon'$ threshold
    crossing (Lemma~\ref{lem:opbound}, existential form) \\
\texttt{formal/metastable\_recovery.v}
  & Coq proofs of structural separation, admissible visibility
    failure, and certificate soundness hierarchy \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{fig4_comparison.pdf}
...
$\delta = 0.015$ to 9.4 at $\delta = 0.030$. The minimum of 1 step at
$\delta \geq 0.025$ is the failure mode noted in Section~\ref{sec:limits}:
very rapid degradation can reach $\ecollapse$ before \eqref{eq:critical}
activates.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{fig2_regime.pdf}
...
at \url{https://github.com/dhwcmoore/Negative-Elasticity-and-Structural-Fatigue-in-Runtime-Monitoring};
...
$\min E_r < 0$ for 100\% of R2 runs. (iii) follows from Table~\ref{tab:mrp}:
...
\begin{table}[H]
\centering
\caption{Episode-level monotonicity in R2 runs with $K \geq 2$ episodes
  ($n = 190$, multi-modal benchmark)}
\label{tab:mrp}
\begin{tabular}{lrr}
\toprule
\textbf{Condition} & \textbf{Count} & \textbf{Fraction} \\
\midrule
...
Separation and fatigue trend conditions hold for majorities; elasticity
...
finite enrichment capacity of the observational boundary itself.
...
restricted to a rate-limited API surface; immutable variants
(Table~\ref{tab:enrichment}) preserve the monitoring contract without
exposing the weight matrix. A full adversarial analysis is a defined
...
Classical RV monitors execution traces against temporal
specifications~\cite{leucker2009brief,havelund2018rule}. STAK-PSAL
monitors the \emph{reduction operator} $M_t$ rather than the output
trace, and anticipates violation before it is confirmed.

\textbf{Drift detection.}
Conventional drift monitors evaluate deviation in current observation
space. STAK-PSAL instead evaluates the cumulative structural cost
required to maintain admissibility under repeated destabilisation.

\textbf{Predictive and parametric monitoring.}
Parametric monitoring~\cite{chen2009monitoring} and predictive
monitoring~\cite{zhao2021predictive} are the closest RV analogues;
this paper instantiates predictive monitoring for admissibility
degradation, with $\mathcal{F}_n$ as the domain-specific degradation
functional.

\textbf{Monitorability and observability.}
The monitorability question~\cite{francalanza2017theory,bauer2011runtime,falcone2010predictive}
asks what properties are monitorable and with what guarantee;
STAK-PSAL specialises this to admissibility degradation, answering
affirmatively for continuous degradation trajectories. Output-layer
monitoring~\cite{tuncali2018simulation,seshia2018verified} assesses
whether outputs are in-distribution; STAK-PSAL monitors $M_t$
directly.

\textbf{Cyber-physical systems, runtime assurance, and PHM.}
Runtime assurance for CPS~\cite{seshia2015towards,schierman2015runtime}
establishes runtime properties on autonomous systems with continuous
dynamics; STAK-PSAL extends these patterns to the observation layer.
Prognostics and Health Management encompasses degradation
indicators~\cite{chiang2014fault} and latent fatigue
accumulation~\cite{jardine2006review}; $\mathcal{F}_n$ specialises the
PHM fatigue functional to observational admissibility. The ecological
distinction between bouncing back and structural
recovery~\cite{pimm1984complexity} maps directly onto the R2/R3 regime
split.

\textbf{Formal methods and static analysis.}
Static abstract interpretation~\cite{cousot1977abstract} and
CEGAR~\cite{clarke2000counterexample} establish soundness or refinement
properties before deployment. STAK-PSAL monitors whether the runtime
reduction continues to preserve the distinctions on which such
properties depend.
...
zero-false-negative claim is model-specific: very rapid degradation can
cause $d_t \leq \ecollapse$ before \eqref{eq:critical} activates
(minimum lead time of 1 step at $\delta \geq 0.025$).
...
operational meaning of enrichment is undefined. Applicability to
discrete generative systems is an open question.
...
runtime quantity. Structural fatigue is not visible in the current
output alone; metastable closure is detected only when the monitor
retains memory of the path by which closure was reached.
